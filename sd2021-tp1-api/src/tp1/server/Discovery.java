				...
				for (;;) {
					try {
						...
						if (msgElems.length == 2) { // periodic announcement
							...
							knownServices.put(msgElems[0],
									new ImmutablePair<String, Long>(msgElems[1], System.currentTimeMillis()));
...
//		System.out.println("sma name?"+this.serviceName.equals(serviceName)+"name 1:	"+this.serviceName+"name 2:	"+serviceName);
			...
			Iterator<String> labelIterator = knownServices.keySet().iterator();

//			System.out.println("list of services");

			while (labelIterator.hasNext()) {
				String label = labelIterator.next();
				if(label.contains(serviceName))
				labels.add(label);
			}
			serverURIs = new URI[labels.size()];
			int index = 0;
			for(String label: labels) {
				//CAN CHECK TIMESTAMP
				serverURIs[index]=URI.create(knownServices.get(label).getLeft());
				index++;
			}
...
//				System.out.println(serverURIs[i]);
	...
	/*
	 * // Main just for testing purposes public static void main( String[] args)
	 * throws Exception { Discovery discovery = new Discovery( DISCOVERY_ADDR,
	 * "test", "http://" + InetAddress.getLocalHost().getHostAddress());
	 * discovery.start(); //TODO // discovery.knownUrisOf(serviceName); }
	 */
}
